"""
@author: xuji0003
@file: music_player_1_0_dev.py
@Description: 音乐盒1.0版本，风格大变
@time: 2021/6/17
"""
...
class MusicPlayer:
    ...
    def selectfile(self):
        v.set(1)
        keywords = entry.get()
        init(keywords, pageNo=v.get(), outdir='.\songCache')
    ...
    def downloadAndPlay(self):
        ...
        if song_file_path.split('.')[-1] == 'm4a':
            ...
            ffmpeg_command = 'ffmpeg -y "{}" -i "{}" -codec:a libmp3lame -qscale:a 1'.format(outfile_name,
                                                                                             song_file_path)
...
if __name__ == '__main__':
    ...
    def show_admire():
        admire_img = Image.open('static/admire.png')
    ...
    pygame.init()
    root.title("小尼音乐")
    root.iconbitmap(r'static/qiaoba.ico')
    ...
    selectImage = Image.open('static/query.png').resize((40, 40))
    ...
    pauseImage = Image.open('static/pause.png').resize((36, 36))
    pausePhoto = ImageTk.PhotoImage(pauseImage)

    playImage = Image.open('static/play.png').resize((35, 35))
    ...
    stopImage = Image.open('static/stop.png').resize((32, 32))
    ...
    lastImage = Image.open('static/last.png').resize((36, 36))
    ...
    nextImage = Image.open('static/next.png').resize((36, 36))
    ...
    # 2
    randomImage = Image.open('static/random.png').resize((35, 35))
    randomPhoto = ImageTk.PhotoImage(randomImage)

    # 1
    singleImage = Image.open('static/single.png').resize((35, 35))
    singlePhoto = ImageTk.PhotoImage(singleImage)

    # 0
    orderImage = Image.open('static/order.png').resize((35, 35))
    ...
    muteImage = Image.open('static/mute.png').resize((35, 35))
    mutePhoto = ImageTk.PhotoImage(muteImage)

    volumeImage = Image.open('static/volume.png').resize((35, 35))
